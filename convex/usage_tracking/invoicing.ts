...
export const generateInvoices = internalMutation({
    ...
    handler: async (ctx, args) => {
        ...
        const result = await ctx.db
            .query('rawUsage')
            .withIndex('billingPeriod_userId', (q) => q.eq('billingPeriod', billingPeriod))
            .paginate({
        ...
        for (const doc of result.page) {
            const cachedPromptTokens = doc.providerMetadata?.openai?.cachedPromptTokens ?? 0;
            const tokens = {
        ...
        if (result.isDone) {
            ...
            await ctx.runMutation(internal.usage_tracking.invoicing.generateInvoices, {
                billingPeriod,
...
const PRICING: Record<string, Record<string, { inputPrice: number; cachedInputPrice: number; outputPrice: number }>> = {
    'openai.chat': {
        'gpt-4o-mini': {
            inputPrice: 0.3,
...
async function createInvoice(
    ...
    invoice: {
        userId: string;
        usage: Record<string, Record<string, { inputTokens: number; outputTokens: number; cachedInputTokens: number }>>;
    },
    ...
    for (const provider of Object.keys(invoice.usage)) {
        for (const model of Object.keys(invoice.usage[provider])) {
            ...
            const { inputPrice, cachedInputPrice, outputPrice } = PRICING[provider][model];
            const { inputTokens, cachedInputTokens, outputTokens } = invoice.usage[provider][model];
            amount +=
    ...
    const existingInvoice = await ctx.db
        .query('invoices')
        .withIndex('billingPeriod_userId', (q) => q.eq('billingPeriod', billingPeriod).eq('userId', invoice.userId))
        .filter((q) => q.neq(q.field('status'), 'failed'))
        .first();
    if (existingInvoice) {
        console.error(`Invoice already exists for ${invoice.userId} ${billingPeriod}`);
    } else {
