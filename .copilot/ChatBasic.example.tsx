...
import { optimisticallySendMessage, toUIMessages, type UIMessage, useThreadMessages } from '@convex-dev/agent/react';
...
export default function ChatBasic() {
    ...
    const threadDetails = useQuery(api.threads.getThreadDetails, threadId ? { threadId } : 'skip');

    // Fetch all threads (internal API)
    // For demo, hardcode userId as in backend
    const threads = usePaginatedQuery(api.threads.listThreads, {}, { initialNumItems: 20 });
        ...
        <div className='h-full flex flex-col'>
            <header className='bg-white/80 backdrop-blur-sm p-4 flex justify-between items-center border-b'>
                <div className='flex items-center gap-4'>
                    <h1 className='text-xl font-semibold accent-text'>Basic Chat Example</h1>
                                ...
                                <li key={thread._id}>
                                    <button
                                        className={cn(
                                            ...
                                            threadId === thread._id && 'bg-blue-100 text-blue-900 font-semibold',
                                        )}
                                        onClick={() => {
...
function Chat({ threadId }: { threadId: string }) {
    const messages = useThreadMessages(api.chat.basic.listMessages, { threadId }, { initialNumItems: 10 });
    const sendMessage = useMutation(api.chat.basic.sendMessage).withOptimisticUpdate(
    ...
    function onSendClicked() {
        ...
        void sendMessage({ threadId, prompt: trimmedPrompt }).catch(() => setPrompt(prompt));
        setPrompt('');
    }
